...
// @version      1
// @description  Broadcast NATs messages whenever a beat occurs while using chordify
...
(function () {
    "use strict";
      ...
      //----------------------------------------------
      // from NATS.io
      // not sure how to include this, except to just paste it here
      // i could not get this to work above:
      // // // @require      https://www.kundalinisoftware.com/cdn/nats
  ...
  class NatsError extends Error {
    ...
    isAuthError() {
      return (
        this.code === ErrorCode.AuthenticationExpired ||
        ...
        while (true) {
          ...
          for (let i10 = 0; i10 < yields.length; i10++) {
            const ok = this.protocolFilterFn
              ? this.protocolFilterFn(yields[i10])
              : true;
            if (ok) {
              this.processed++;
              yield yields[i10];
  ...
  function canonicalMIMEHeaderKey(k) {
    ...
    for (let i11 = 0; i11 < k.length; i11++) {
      ...
      if (c === 58 || c < 33 || c > 126) {
        throw new NatsError(
          `'${k[i11]}' is not a valid character for a header key`,
          ErrorCode.BadHeader
        );
      }
      if (upper && 97 <= c && c <= 122) {
  ...
  function dtoi(s) {
    ...
    for (
      i20 = 0;
      i20 < s.length && 48 <= s.charCodeAt(i20) && s.charCodeAt(i20) <= 57;
      i20++
    ) {
  ...
  class SubscriptionImpl extends QueuedIteratorImpl {
    ...
    setPrePostHandlers(opts) {
      if (this.noIterator) {
        ...
        const filter = opts.protocolFilterFn
          ? opts.protocolFilterFn
          : () => {
              return true;
            };
            ...
            switch (b) {
              ...
              case cc.NL: {
                const arg = this.argBuf
                  ? this.argBuf.bytes()
                  : buf.subarray(this.as, i23 - this.drop);
  ...
  class ProtocolHandler {
    ...
    constructor(options, publisher) {
      ...
      const servers =
        typeof options.servers === "string" ? [options.servers] : options.servers;
    ...
    function encodeChunk(buf, start, end) {
      ...
      for (let i27 = start, curTriplet = 0; i27 < end; i27 += 3) {
        out[curTriplet++] = tripletToBase64(
          (buf[i27] << 16) + (buf[i27 + 1] << 8) + buf[i27 + 2]
        );
      }
      return out.join("");
    }
    return {
      ...
      toUint8Array(b64) {
        ...
        for (i28 = 0; i28 < len; i28 += 4) {
          tmp =
            (revLookup1[b64.charCodeAt(i28)] << 18) |
            (revLookup1[b64.charCodeAt(i28 + 1)] << 12) |
            (revLookup1[b64.charCodeAt(i28 + 2)] << 6) |
            revLookup1[b64.charCodeAt(i28 + 3)];
        ...
        if (placeHoldersLen === 2) {
          tmp =
            (revLookup1[b64.charCodeAt(i28)] << 2) |
            (revLookup1[b64.charCodeAt(i28 + 1)] >> 4);
          buf[curByte++] = tmp & 255;
        } else if (placeHoldersLen === 1) {
          tmp =
            (revLookup1[b64.charCodeAt(i28)] << 10) |
            (revLookup1[b64.charCodeAt(i28 + 1)] << 4) |
            (revLookup1[b64.charCodeAt(i28 + 2)] >> 2);
          buf[curByte++] = (tmp >> 8) & 255;
          buf[curByte++] = tmp & 255;
        }
        return buf;
  ...
  function unpack25519(o, n) {
    for (let i38 = 0; i38 < 16; i38++)
      o[i38] = n[2 * i38] + (n[2 * i38 + 1] << 8);
    o[15] &= 32767;
  }
  ...
  function sign_detached_verify(msg, sig, publicKey) {
    ...
    for (i43 = 0; i43 < msg.length; i43++)
      sm[i43 + SignLength.Signature] = msg[i43];
  ...
  (function (SealedBoxLength1) {
    SealedBoxLength1[(SealedBoxLength1["PublicKey"] = BoxLength.PublicKey)] =
      "PublicKey";
    SealedBoxLength1[(SealedBoxLength1["SecretKey"] = BoxLength.SecretKey)] =
      "SecretKey";
    SealedBoxLength1[(SealedBoxLength1["Nonce"] = BoxLength.Nonce)] = "Nonce";
    SealedBoxLength1[
      (SealedBoxLength1["Overhead"] = BoxLength.PublicKey + BoxLength.Overhead)
    ] = "Overhead";
  ...
  (function (NKeysErrorCode1) {
    ...
    NKeysErrorCode1["CannotSign"] =
      "nkeys: cannot sign, no private key available";
  ...
  class Metric {
    ...
    toString() {
      ...
      return `${this.name}${label ? " [asyncRequests]" : ""} ${humanizeNumber(
        mps
      )} msgs/sec - [${sec.toFixed(2)} secs] ~ ${throughput(
        this.bytes,
        sec
      )} ${minmax}`;
    }
    toCsv() {
      return `"${this.name}",${new Date(this.date).toISOString()},${this.lang},${
        this.version
      },${this.msgs},${this.payload},${this.bytes},${this.duration},${
        ...
        const job = (async () => {
          if (this.asyncRequests) {
            ...
            for (let i60 = 0; i60 < this.msgs; i60++) {
              a.push(
                this.nc.request(this.subject, this.payload, {
                  timeout: 20000,
                })
              );
            }
  ...
  function humanizeBytes(bytes, si = false) {
    ...
    return `${(bytes / Math.pow(base1, exp)).toFixed(2)} ${
      pre[index]
    }${post}/sec`;
  }
  ...
  (function (PubHeaders1) {
    ...
    PubHeaders1["ExpectedLastSubjectSequenceHdr"] =
    ...
    fetch(stream, durable, opts = {}) {
      ...
      qi.dispatchedFn = (m) => {
        if (m) {
          ...
          if (
            (qi.getPending() === 1 && m.info.pending === 0) ||
            wants === received
          ) {
            qi.stop();
          }
  ...
  async function connectToServer() {
      ...
      nc = await connect({ servers: "ws://localhost:4223" });
      sc = StringCodec();
  }
  
  connectToServer();
  
  // since the chordify chords are populated at runtime...
  // the simplest way i could find to do it is to just wait 10 seconds for them to show up
  const handle = setTimeout(mainRoutine, 10000);
  ...
  function mainRoutine() {
      ...
      // add a mutationObserver to each chord
      Array.from(chords.children).forEach(
        function(element) {
  
  
  
            observer[i] = new MutationObserver(
              function (event) {
  
  
  
                // check to see if its classname was just set to include 'currentChord'
                if (event[0].target.className.includes("currentChord")) {
  
                    //let chordName1 = event[0].target.children[0].children[0];
                    let chordName = event[0].target.children[0].children[0].innerHTML;
                    //chordName = chordName.replace("chord-label label-", "");
                    console.log(chordName);
                    nc.publish("chord", sc.encode(chordName));
                }
            })
  
            observer[i].observe(element, {attributes: true, attributeOldValue:true} )
            i++;
          }
  );
